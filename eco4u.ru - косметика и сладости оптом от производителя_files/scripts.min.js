...
$(function() {
	...
	$('.index-first-screen-carousel').slick({
		dots: true,
		autoplay: true,
		// fade: true,
		responsive: [
	...
	// Карусель со статьями

	$('.default-articles-carousel').slick({
		dots: true,
		// fade: true,
		responsive: [
			{
	...
	$('.card-main-carousel').slick({
		dots: true,
		// fade: true,
	});

	// Карусель на странице продукции

	$('.production-gallery-carousel').slick({
		dots: true,
		// fade: true,
		responsive: [
			{
	...
	// Аккордеон в фильтре на странице выдачи

	$('.issue-main-filter-block-top').click(function(){
		var $par = $(this).parents('.issue-main-filter-block');
	...
	// Валидация в фильтре на странице выдачи

	$('.issue-main-filter-block-price input').on('input' ,function(){
		if(!pricePattern.test($(this).val())) {
			$(this).addClass('error');
			$('.filter-submit-btn').attr('disabled', true);
		} else {
			$(this).removeClass('error');
			$('.filter-submit-btn').attr('disabled', false);
		}
	});

	$('.issue-main-filter-block-price input:last-child').on('input' ,function(){
		if($(this).val() < $('.issue-main-filter-block-price input:first-child').val()) {
			$(this).val(+$('.issue-main-filter-block-price input:first-child').val() + 1);
		}
	});

	// Сброс фильтра на странице выдачи

	$('.filter-reset-btn').click(function(){
		$('.issue-main-filter form').trigger("reset");
	});

	// Цена в блоке товара в корзине
/*
	function sumPrice(className){
		var sum = 0;
		for(i = 0; i < $(className).length; i++){
			var priceForOne = $(className)[i].innerHTML;
			sum += +priceForOne
		};
		return sum;
	};

	$('.cart-main-bot-price').text(sumPrice('.cart-main-good-price-number'));

	$('.cart-main-good-count input').on('input' ,function(){
		var parents = $(this).parents('.cart-main-good');
		var price = $('.cart-main-good-price input', parents).val();
		var val = $(this).val();
		$('.cart-main-good-price-number', parents).text(+price * +val)
		if(!pricePattern.test($(this).val())) {
			$(this).val(1);
			$('.cart-main-good-price-number', parents).text(+price);
			$('.cart-main-bot-price').text(sumPrice('.cart-main-good-price-number'));
		};
		$('.cart-main-bot-price').text(sumPrice('.cart-main-good-price-number'));
	});
...
/*
	...
	$('.cart-main-good-count-plus-btn').click(function(){
		var parents = $(this).parents('.cart-main-good');
		var price = $('.cart-main-good-price input', parents).val();
		var val = $('.cart-main-good-count input', parents).val();
		$('.cart-main-good-price-number', parents).text(+price * (+val + 1))
		$('.cart-main-good-count input', parents).val(+val + 1);
		$('.cart-main-bot-price').text(sumPrice('.cart-main-good-price-number'));
	});

	// Кнопка - в корзине

	$('.cart-main-good-count-minus-btn').click(function(){
		var parents = $(this).parents('.cart-main-good');
		var price = $('.cart-main-good-price input', parents).val();
		var val = $('.cart-main-good-count input', parents).val();
		$('.cart-main-good-price-number', parents).text(+price * (+val - 1))
		...
		$('.cart-main-bot-price').text(sumPrice('.cart-main-good-price-number'));
		if(val < 2){
			$('.cart-main-good-count input', parents).val(1);
			$('.cart-main-good-price-number', parents).text(+price);
			$('.cart-main-bot-price').text(sumPrice('.cart-main-good-price-number'));
		}
	});
*/
	// Переход на вкладку "Отзывы" в карточке товара

	$('.card-main-desc-reviews-link').click(function(){
		$('#pills-5-tab').tab('show')
	});

	// Аккордеон в заказах в профиле

	$('.profile-main-right-orders-block-top').click(function(){
		var parents = $(this).parents('.profile-main-right-orders-block');
		$(this).toggleClass('active');
		$('.profile-main-right-orders-block-hidden', parents).slideToggle();
	});

	// Поп-ап с корзиной на десктопе

	$('.header-top-cart-btn').click(function(e){
		e.preventDefault();
		$('.header-cart').toggle();
	});

	$('.header-cart-close-btn').click(function(){
		$('.header-cart').hide();
	});

	// Селект в поп-апе с отзывом

	// $('.default-label-select').click(function(){
	// 	$(this).toggleClass('open')
	// });

	// Мобильное меню-бургер

	$('.header-left-circle').click(function(){
		$('.toggle-menu-wrap').toggleClass('on');
		$('.header-mobile-menu').toggleClass('open');
	});

	// Мобильная навигация

	$('.header-mobile-nav-top').click(function(){
		$(this).toggleClass('open')
		$('.header-mobile-nav-hidden').slideToggle();
	});

	// Мобильный аккордон в фильтре на странице выдачи

	$('.issue-main-filter-mobile-top').click(function(){
		$(this).toggleClass('close');
		$('.issue-main-filter-mobile-slide').slideToggle();
	});

	$('.issue-main-right-top-select select').selecter();

	$('.profile-main-right-orders-top-sort select').selecter();

	$('.popup-main-default .default-label select').selecter();
...
/*
	$('.header-cart-main-good-input input').on('input', function(){
		var parents = $(this).parents('.header-cart-main-good');
		var price = $('.header-cart-main-good-price input', parents).val();
		var val = $('.header-cart-main-good-input input', parents).val();
		$('.header-cart-main-good-price span', parents).text(+price * +val);
		if(!pricePattern.test($(this).val())) {
			$(this).val('');
			$('.header-cart-main-good-price span', parents).text('0');
		};
	});
*/
  ...
  let window = $("body").innerWidth()
    $(document).mouseup(function (e) {
      let serch = $('#form-search');
      if (window <= 991){
  ...
  var elems = document.querySelector('.search-form');
